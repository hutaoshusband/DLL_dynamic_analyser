...
fn connect_to_pipe(pipe_name: &str, access: u32, status_arc: Arc<Mutex<String>>) -> Option<isize> {
    ...
    for i in 0..MAX_RETRIES {
        ...
        if pipe_handle != INVALID_HANDLE_VALUE {
            return Some(pipe_handle); // Success
        }

        let err = unsafe { GetLastError() };
        if err != ERROR_PIPE_BUSY {
            *status_arc.lock().unwrap() = format!("Failed to connect to pipe '{}'. Error: {}", pipe_name, err);
            return None;
        }

        *status_arc.lock().unwrap() =
            format!("Pipe '{}' is busy, retrying... ({}/{})", pipe_name, i + 1, MAX_RETRIES);
        thread::sleep(Duration::from_millis(RETRY_DELAY_MS));
    }

    *status_arc.lock().unwrap() = format!("Failed to connect to pipe '{}' after {} retries.", pipe_name, MAX_RETRIES);
    None
}
...
pub fn setup_communication(
    ...
    cmd_pipe_handle_arc: Arc<Mutex<Option<isize>>>,
    log_pipe_handle_arc: Arc<Mutex<Option<isize>>>,
    status_arc: Arc<Mutex<String>>,
    logger: Sender<String>,
) -> bool {
    // Connect to the command pipe (write-only)
    let cmd_pipe_name = format!("\\\\.\\pipe\\cs2_monitor_cmd_{{}}", pid);
    let cmd_pipe_handle = match connect_to_pipe(&cmd_pipe_name, FILE_GENERIC_WRITE, status_arc.clone()) {
        Some(handle) => handle,
        None => return false, // Error already logged by connect_to_pipe
    };
    *cmd_pipe_handle_arc.lock().unwrap() = Some(cmd_pipe_handle);

    // Connect to the log pipe (read-only)
    let log_pipe_name = format!("\\\\.\\pipe\\cs2_monitor_log_{{}}", pid);
    let log_pipe_handle = match connect_to_pipe(&log_pipe_name, FILE_GENERIC_READ, status_arc.clone()) {
        Some(handle) => handle,
        None => return false, // Error already logged by connect_to_pipe
    };
    *log_pipe_handle_arc.lock().unwrap() = Some(log_pipe_handle);

    // --- Send Initial Configuration via Command Pipe ---
    ...
    *status_arc.lock().unwrap() = "Configuration sent. Monitoring...".to_string();
    ...
    // --- Start Log Listener on Log Pipe ---
    start_pipe_log_listener(log_pipe_handle, logger);
